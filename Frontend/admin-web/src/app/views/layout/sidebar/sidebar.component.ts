...
export class SidebarComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    // If permissions exist in session or not
    if (localStorage.getItem("permissions") === null) {
      new Promise(() => {
        setTimeout(() => this.menuItems = this.setIsVisibleMenuItems(MENU), 1000);
      });

    } else {

      this.menuItems = this.setIsVisibleMenuItems(MENU);
    }
  ...
  setIsVisibleMenuItems(menu: any) {
    ...
    menu.forEach((item: any) => {
      ...
      if(item.hasOwnProperty('data')){
        ...
        item.data?.forEach((e: string) => {
          if (permissions.find((x: string) => x === e)) {
            status = true;
            return;
          }
        });
